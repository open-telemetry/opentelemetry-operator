    ...
    config:
      ...
      - job_name: otel-collector
        ...
        - action: labelmap
          ...
          replacement: $1 
```
Note that in this case, the Operator replaces "$$" with a single "$" symbol. This is because the collector supports environment variable substitution, whereas the TA (Target Allocator) does not. Therefore, to ensure compatibility, the TA ConfigMap should only contain a single "$" symbol.
