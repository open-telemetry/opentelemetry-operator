...
This will create an OpenTelemetry Collector instance named `otel`, exposing two services `simplest-collector` and `simples-collector-headless`. The received spans will be exported via `logging` exporter and written to the console (`stdout`). If a gRPC protocol is used to export data to the collector, use the headless service to ensure that load balancing will properly work. In gRPC load balancing has to be configured explicitly in the client e.g. by using `round_robin` configuration.
...
spec:
  exporter:
    endpoint: http://simplest-collector-headless:4317
  propagators:
    - tracecontext
    - baggage
    - b3
