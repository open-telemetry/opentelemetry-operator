...
## Cosign signing and verification notes

The supply-chain workflow behaves a little differently when it runs locally versus in GitHub Actions:

- In GitHub Actions, keyless signing uses OIDC automatically.
- Locally, `cosign` keyless signing requires you to authenticate manually in the browser.
- The signature is not stored locally; it is recorded in a transparency log such as Rekor.

When verifying a release image, the expected identity and issuer are tied to GitHub Actions:

- Expected issuer: `https://token.actions.githubusercontent.com`
- Expected identity: the `publish-images.yaml` workflow running on a tag ref

If verification fails locally with a message like `none of the expected identities matched what was in the certificate`, that usually means the artifact was signed with a local browser login rather than the GitHub Actions identity. In that case, check the certificate subject/issuer first before looking for a workflow problem.

Useful local checks:

- `make cosign-sign IMAGE=... DIGEST=...`
- `make security-artifacts IMAGE=... DIGEST=... RELEASE_TAG=...`
- `make sbom-scan RELEASE_TAG=...`
- `make supply-chain-verify IMAGE=... DIGEST=... GITHUB_REPOSITORY=open-telemetry/opentelemetry-operator`

Generated files and what they are:

- `opentelemetry-operator-${RELEASE_TAG}.intoto.jsonl`  the SLSA provenance attestation downloaded from the registry; it records how the image was built and signed.
- `opentelemetry-operator-${RELEASE_TAG}.sbom.spdx.json`  the SPDX SBOM generated by Syft; it lists the packages, files, and metadata found in the image.
- `opentelemetry-operator-${RELEASE_TAG}.sbom.cyclonedx.json`  the CycloneDX SBOM generated by Syft; it contains the same software inventory in CycloneDX format.
- `grype-results.txt`  the vulnerability scan output produced from the SPDX SBOM when running the workflow locally.

One more detail: `make cosign-sign` does not leave behind a local signature file. The signature is attached to the image digest in the container registry and recorded in the transparency log.

Common verification failures:

- If `make supply-chain-verify` fails locally with an identity mismatch, compare the certificate subject/issuer against the workflow expectations in `.github/workflows/publish-images.yaml`.
- A local browser login typically produces a subject like an email address and an issuer like `https://github.com/login/oauth`, which will not match the GitHub Actions verification rules.
- If the provenance check fails, make sure the image digest, repository name, and tag all refer to the same release artifact.
