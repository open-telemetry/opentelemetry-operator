...
func (c CollectorWebhook) defaulter(r *OpenTelemetryCollector) error {
	...
	if r.Spec.TargetAllocator.Enabled && r.Spec.TargetAllocator.Common.Replicas == nil {
		r.Spec.TargetAllocator.Common.Replicas = &one
	}

	if r.Spec.MaxReplicas != nil || (r.Spec.Common.Autoscaler != nil && r.Spec.Common.Autoscaler.MaxReplicas != nil) {
		if r.Spec.Common.Autoscaler == nil {
			r.Spec.Common.Autoscaler = &AutoscalerSpec{}
		}

		if r.Spec.Common.Autoscaler.MaxReplicas == nil {
			r.Spec.Common.Autoscaler.MaxReplicas = r.Spec.MaxReplicas
		}
		if r.Spec.Common.Autoscaler.MinReplicas == nil {
			if r.Spec.MinReplicas != nil {
				r.Spec.Common.Autoscaler.MinReplicas = r.Spec.MinReplicas
			} else {
				r.Spec.Common.Autoscaler.MinReplicas = r.Spec.Common.Replicas
			}
		}

		if r.Spec.Common.Autoscaler.TargetMemoryUtilization == nil && r.Spec.Common.Autoscaler.TargetCPUUtilization == nil {
			defaultCPUTarget := int32(90)
			r.Spec.Common.Autoscaler.TargetCPUUtilization = &defaultCPUTarget
		}
	}
	...
	if r.Spec.Common.PodDisruptionBudget == nil {
		r.Spec.Common.PodDisruptionBudget = &PodDisruptionBudgetSpec{
	...
	if len(r.Spec.Common.ManagementState) == 0 {
		r.Spec.Common.ManagementState = ManagementStateManaged
	}
	return nil
}
...
func (c CollectorWebhook) validate(r *OpenTelemetryCollector) (admission.Warnings, error) {
	...
	if r.Spec.Mode != ModeStatefulSet && len(r.Spec.Common.VolumeClaimTemplates) > 0 {
	...
	if r.Spec.Mode == ModeSidecar && len(r.Spec.Common.Tolerations) > 0 {
	...
	if r.Spec.Mode == ModeSidecar && r.Spec.Common.PriorityClassName != "" {
	...
	// validate affinity
	if r.Spec.Mode == ModeSidecar && r.Spec.Common.Affinity != nil {
	...
	if r.Spec.Common.Autoscaler != nil && r.Spec.Common.Autoscaler.MaxReplicas != nil {
		maxReplicas = r.Spec.Common.Autoscaler.MaxReplicas
	}
	...
	if r.Spec.Common.Autoscaler != nil && r.Spec.Common.Autoscaler.MinReplicas != nil {
		minReplicas = r.Spec.Common.Autoscaler.MinReplicas
	}
	...
	if minReplicas == nil {
		if r.Spec.MinReplicas != nil {
			...
			minReplicas = r.Spec.Common.Replicas
		}
	}
	...
	if maxReplicas != nil {
		...
		if r.Spec.Common.Replicas != nil && *r.Spec.Common.Replicas > *maxReplicas {
		...
		if r.Spec.Common.Autoscaler != nil {
			return warnings, checkAutoscalerSpec(r.Spec.Common.Autoscaler)
		}
	}
