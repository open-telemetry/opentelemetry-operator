# Changelog — Node.js auto-instrumentation image

All notable changes to the OpenTelemetry Node.js auto-instrumentation image
(`ghcr.io/open-telemetry/opentelemetry-operator/autoinstrumentation-nodejs`) are
recorded here, one entry per published `<sdk-version>-<revision>` image tag. See
[../README.md](../README.md) for the tagging scheme.

Entries are added automatically by the autoinstrumentation-revision tooling when
the image's SDK version or contents change; see [../README.md](../README.md).

## 0.78.0-4

- Rebuild Node.js auto-instrumentation image (base image or dependency update). (#5633)

## 0.78.0-3

- Initial changelog entry for the OpenTelemetry Node.js auto-instrumentation image.
