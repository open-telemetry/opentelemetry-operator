	...
	tests := []struct {
		...
		},
		{
			...
			wantErr: func(t assert.TestingT, err error, i ...any) bool {
				...
				// LoadFromFile validates for duplicate standalone config keys
				// (yamlv3) before unmarshaling the rest of the file (yaml.v2),
				// so a malformed YAML may surface as either error.
				msg := err.Error()
				if !strings.Contains(msg, "error unmarshaling YAML") && !strings.Contains(msg, "error parsing YAML") {
...
func TestValidateNoDuplicateStandaloneConfigKeys(t *testing.T) {
	cfg := []byte(`
mode: standalone
standalone:
  agents:
...
`)
	...
	err := validateNoDuplicateStandaloneConfigKeys(cfg)
	require.Error(t, err)
	assert.ErrorContains(t, err, `standalone agent "collector" config key "collector" is duplicated`)
}
...
func TestForStandaloneAgentUsesWorkloadRefNameAsHostName(t *testing.T) {
	cfg := NewConfig(logr.Discard())
	cfg.Mode = standaloneMode

	agentCfg := cfg.ForStandaloneAgent(StandaloneAgentConfig{
		Namespace: "default",
		Type:      "otel-collector",
