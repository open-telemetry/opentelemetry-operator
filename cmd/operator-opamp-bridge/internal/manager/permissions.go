...
func CheckPermissions(ctx context.Context, k8sClient PermissionReviewClient, perms []config.Permission) error {
	if len(perms) == 0 {
		return nil
	}
	if k8sClient == nil {
...
func checkPermission(ctx context.Context, k8sClient PermissionReviewClient, perm config.Permission) error {
