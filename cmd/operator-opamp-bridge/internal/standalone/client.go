...
func (c *Client) Apply(name, namespace string, configFile *protobufs.AgentConfigFile) error {
	...
	if err := received.validateCollectorConfig(); err != nil {
		return fmt.Errorf("invalid standalone config: %w", err)
	}
...
// Delete reports that standalone mode does not support remote ConfigMap deletion.
func (*Client) Delete(name, namespace string) error {
	return fmt.Errorf("standalone mode does not support deleting ConfigMap %s/%s", namespace, name)
}
...
func (c *Client) ListInstances() ([]operator.CollectorInstance, error) {
	...
	for i := range cmList.Items {
		...
		result = append(result, newStandaloneCollectorInstance(cm.Name, cm.Namespace, cm.GetCreationTimestamp().Time, configBody))
	}
	return result, nil
}
...
func (c *Client) triggerRollout(ctx context.Context, namespace string, workloads []string) error {
	...
	for _, ref := range workloads {
		...
		case "statefulset":
			...
			if err := c.k8sClient.Get(ctx, client.ObjectKey{Name: name, Namespace: namespace}, sts); err != nil {
				c.log.Error(err, "failed to get StatefulSet for rollout", "name", name, "namespace", namespace)
				continue
			}
			...
			if err := c.k8sClient.Update(ctx, sts); err != nil {
				c.log.Error(err, "failed to trigger rollout for StatefulSet", "name", name, "namespace", namespace)
			}
		default:
