...
func validConfigMapData() map[string]string {
	return map[string]string{"collector.yaml": validCollectorConfig}
}
...
func testStatefulSet(name string) *appsv1.StatefulSet {
	replicas := int32(2)
...
func TestApply_RejectsUnmanagedConfigMap(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := c.Apply("my-config", "default", &protobufs.AgentConfigFile{
				Body:        mustMarshalStandaloneConfig(t, "my-config", "default", validConfigMapData()),
				ContentType: "yaml",
			})
			require.Error(t, err)
...
func TestApply_TriggersStatefulSetRollout(t *testing.T) {
	cm := managedConfigMap("my-config", "default", "StatefulSet/my-collector")
	sts := testStatefulSet("my-collector")
	k8s := getFakeK8sClient(t, cm, sts)
	c := newTestClient(k8s)
...
func TestApply_TriggersMultipleWorkloadRollouts(t *testing.T) {
	cm := managedConfigMap("my-config", "default", "Deployment/col-a,DaemonSet/agent-b,StatefulSet/col-c")
	deploy := testDeployment("col-a")
	ds := testDaemonSet("agent-b")
	sts := testStatefulSet("col-c")
	k8s := getFakeK8sClient(t, cm, deploy, ds, sts)
	c := newTestClient(k8s)
	...
	require.NoError(t, k8s.Get(context.Background(), client.ObjectKey{Name: "col-c", Namespace: "default"}, updatedSTS))
	assert.Contains(t, updatedSTS.Spec.Template.Annotations, restartAnnotation)
}
...
func TestApply_AcceptsAnyValidCollectorConfigEntry(t *testing.T) {
	tests := []struct {
		name string
		data map[string]string
	}{
		{
			name: "valid collector.yaml",
			data: validConfigMapData(),
		},
		{
			name: "valid custom key",
			data: map[string]string{"custom-collector.yaml": validCollectorConfig},
		},
		{
			name: "one invalid key and one valid key",
			data: map[string]string{
				"notes.txt":      "not collector config",
				"collector.yaml": validCollectorConfig,
			},
		},
		{
			name: "pipeline references do not need to resolve",
			data: map[string]string{
...
`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := c.Apply("my-config", "default", &protobufs.AgentConfigFile{
				Body:        mustMarshalStandaloneConfig(t, "my-config", "default", tt.data),
				ContentType: "yaml",
			})
			require.NoError(t, err)
			...
			cm := &v1.ConfigMap{}
			require.NoError(t, k8s.Get(context.Background(), client.ObjectKey{Name: "my-config", Namespace: "default"}, cm))
			assert.Equal(t, tt.data, cm.Data)
		})
	}
}

func TestApply_RejectsWhenNoCollectorConfigEntryIsValid(t *testing.T) {
	tests := []struct {
		name          string
		data          map[string]string
		errorContains string
		...
		{
			name:          "empty value",
			data:          map[string]string{"collector.yaml": "  \n"},
			errorContains: "config value is empty",
		},
		{
			name:          "invalid YAML",
			data:          map[string]string{"collector.yaml": "receivers:\n  -"},
			errorContains: "failed to parse collector config",
		},
		{
...
`,
			...
			errorContains: "must define at least one receiver",
		},
		{
...
`,
			...
			errorContains: "must define at least one exporter",
		},
		{
...
`,
			...
			errorContains: "must define at least one service pipeline",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.Error(t, err)
			assert.Contains(t, err.Error(), "no valid OpenTelemetry Collector config found")
			assert.Contains(t, err.Error(), tt.errorContains)
		})
	}
}
...
func TestApply_MissingWorkloadIsSkipped(t *testing.T) {
	...
	err := c.Apply(configName, ns, &protobufs.AgentConfigFile{
		Body:        mustMarshalStandaloneConfig(t, configName, ns, validConfigMapData()),
		ContentType: "yaml",
	})
	...
	assert.Equal(t, validConfigMapData(), updated.Data)
}
...
func TestApply_MalformedWorkloadReferenceIsSkipped(t *testing.T) {
	...
	assert.Equal(t, validConfigMapData(), updatedCM.Data)
...
func TestApply_UnsupportedWorkloadKindIsSkipped(t *testing.T) {
	cm := managedConfigMap("my-config", "default", "ReplicaSet/my-collector,Deployment/my-collector")
	deploy := testDeployment("my-collector")
	k8s := getFakeK8sClient(t, cm, deploy)
	c := newTestClient(k8s)
...
func TestDelete_Unsupported(t *testing.T) {
	...
	require.Error(t, err)
	assert.Contains(t, err.Error(), "standalone mode does not support deleting ConfigMap default/my-config")
...
func TestDelete_NotFound(t *testing.T) {
	...
	err := c.Delete("nonexistent", "default")
	require.Error(t, err)
	assert.Contains(t, err.Error(), "standalone mode does not support deleting ConfigMap default/nonexistent")
}
...
func TestListInstances_EffectiveConfigContainsData(t *testing.T) {
	...
	assert.Equal(t, "configmap/default/my-config", instances[0].GetConfigMapKey().String())
