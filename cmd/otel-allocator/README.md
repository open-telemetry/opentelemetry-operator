...
Note that enabling this also breaks monitors that legitimately need file-based credentials, API server and kubelet scraping among them. Those endpoints must move to a secret-based credential — see [Service / Pod monitor endpoint credentials](#service--pod-monitor-endpoint-credentials) — or the setting has to stay off.
