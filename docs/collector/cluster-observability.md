...
transition-only `ComponentsNotReady` warning when a current rollout fails. It
also reports `Conflicted` when another ClusterObservability first loses the
singleton election. Successful updates, progressing rollouts, recoveries, and
repeated observations of the same state do not emit events.
...
Events:
  ...
  ----     ------      ---  ----                   -------
  Warning  Conflicted  4s   cluster-observability  Multiple ClusterObservability resources detected. Only the oldest resource is active.
```
