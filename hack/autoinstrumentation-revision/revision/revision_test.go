...
func TestChangelogNewImageIsNoOp(t *testing.T) {
	...
	repo := Repo{Root: root, Git: fakeGit{base: map[string]string{}, changed: []string{"autoinstrumentation/java/Dockerfile"}}}
	...
	if len(entries) != 0 {
		t.Errorf("expected no entry when base SDK is empty, got %+v", entries)
	}
}
