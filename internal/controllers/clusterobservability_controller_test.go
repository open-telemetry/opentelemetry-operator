...
func TestClusterObservabilityTopologyChangesEnqueueAllInstances(t *testing.T) {
	const namespace = "observability"
	scheme := runtime.NewScheme()
	require.NoError(t, v1alpha1.AddToScheme(scheme))
	winner := &v1alpha1.ClusterObservability{ObjectMeta: metav1.ObjectMeta{Name: "winner", Namespace: namespace}}
	remaining := &v1alpha1.ClusterObservability{ObjectMeta: metav1.ObjectMeta{Name: "remaining", Namespace: namespace}}
	cli := fake.NewClientBuilder().WithScheme(scheme).WithObjects(winner, remaining).Build()
	reconciler := NewClusterObservabilityReconciler(ClusterObservabilityReconcilerParams{
		Client: cli,
		Log:    logr.Discard(),
	})

	predicate := clusterObservabilityTopologyPredicate()
	assert.True(t, predicate.Create(event.CreateEvent{Object: remaining}))
	assert.True(t, predicate.Delete(event.DeleteEvent{Object: winner}))
	assert.False(t, predicate.Update(event.UpdateEvent{ObjectOld: remaining, ObjectNew: remaining.DeepCopy()}))
	assert.False(t, predicate.Generic(event.GenericEvent{Object: remaining}))

	require.NoError(t, cli.Delete(context.Background(), winner))
	requests := reconciler.findAllClusterObservability(context.Background(), winner)
	require.Len(t, requests, 1)
	assert.Equal(t, types.NamespacedName{Name: remaining.Name, Namespace: namespace}, requests[0].NamespacedName)
}
