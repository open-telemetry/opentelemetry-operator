...
var _ = Describe("PodInjector", func() {
	...
	It("should inject an appropriate sidecar", func() {
		for _, tt := range []struct {
			...
			},
			{
				...
				name: "auto-select basde on the annotation's value",
				ns: corev1.Namespace{
					ObjectMeta: metav1.ObjectMeta{
			...
			When(tt.name, func() {
				...
				Expect(res.AdmissionResponse.Result).To(BeNil())
				...
				for k := range expectedMap {
					Expect(expectedMap[k]).To(BeTrue(), "patch with path %s not found", k)
				}

				// cleanup
				Expect(k8sClient.Delete(context.Background(), &tt.otelcol)).ToNot(HaveOccurred())
				Expect(k8sClient.Delete(context.Background(), &tt.ns)).ToNot(HaveOccurred())
			})
		}
	})

	It("pod should not be changed", func() {
		for _, tt := range []struct {
			name     string
			ns       corev1.Namespace
			pod      corev1.Pod
	...
	It("it should fail when the request is invalid", func() {
		...
		for _, tt := range []struct {
			...
			When(tt.name, func() {
				...
				Expect(res.AdmissionResponse.Result).ToNot(BeNil())
				Expect(res.AdmissionResponse.Result.Code).To(Equal(tt.expected))
			})
		}
	})
})
