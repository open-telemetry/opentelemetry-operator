...
// ErrMultipleInstances indicates that another ClusterObservability resource won
// the singleton election. It is not returned to the controller-runtime workqueue.
var ErrMultipleInstances = errors.New("multiple ClusterObservability resources detected")
...
// isConflictError checks if the error indicates a singleton conflict.
...
func emitStatusEvents(
	...
	if isConflicted {
		previousConflicted := findCondition(previous.Status.Conditions, v1alpha1.ClusterObservabilityConditionConflicted)
		conflicted := findCondition(changed.Status.Conditions, v1alpha1.ClusterObservabilityConditionConflicted)
		if conflicted != nil && conflicted.Status == metav1.ConditionTrue &&
			(previousConflicted == nil || previousConflicted.Status != metav1.ConditionTrue) {
			params.Recorder.Eventf(changed, nil, corev1.EventTypeWarning, reasonConflicted, reasonConflicted,
