...
type Version struct {
	...
	SplunkCollector string `json:"splunk-otel-collector-version"`
...
func Get() Version {
	return Version{
		Operator:        version,
		BuildDate:       buildDate,
		SplunkCollector: OpenTelemetryCollector(),
		Go:              runtime.Version(),
...
func (v Version) String() string {
	return fmt.Sprintf(
		"Version(Operator='%v', BuildDate='%v', SplunkOpenTelemetryCollector='%v', Go='%v', TargetAllocator='%v')",
		v.Operator,
		v.BuildDate,
		v.SplunkCollector,
		v.Go,
...
// OpenTelemetryCollector returns the default Splunk OpenTelemetryCollector to use when no versions are specified via CLI or configuration.
func OpenTelemetryCollector() string {
	if len(splunkOtelCol) > 0 {
		// this should always be set, as it's specified during the build
		return splunkOtelCol
	}
