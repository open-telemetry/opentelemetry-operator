...
func TestIsInstrumentationVersionUnupgradable(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:         "upgrade across blocked version is blocked",
			language:     constants.InstrumentationLanguageJava,
			image:        "ghcr.io/org/java:1.0.0",
			defaultImage: "ghcr.io/org/java:3.0.0",
			unupgradableMap: map[constants.InstrumentationLanguage]map[string]string{
				constants.InstrumentationLanguageJava: {
					"2.0.0": "Breaking change at v2.",
				},
			},
			wantBlocked: true,
			wantMessage: "Breaking change at v2.",
		},
		{
			name:         "blocked version above target does not block",
			language:     constants.InstrumentationLanguageJava,
			image:        "ghcr.io/org/java:1.0.0",
			defaultImage: "ghcr.io/org/java:1.5.0",
			unupgradableMap: map[constants.InstrumentationLanguage]map[string]string{
				constants.InstrumentationLanguageJava: {
					"2.0.0": "Should not block.",
				},
			},
			wantBlocked: false,
		},
		{
			name:         "blocked version below current does not block",
			language:     constants.InstrumentationLanguageJava,
			image:        "ghcr.io/org/java:2.0.0",
			defaultImage: "ghcr.io/org/java:3.0.0",
			unupgradableMap: map[constants.InstrumentationLanguage]map[string]string{
				constants.InstrumentationLanguageJava: {
					"1.0.0": "Should not block.",
				},
			},
			wantBlocked: false,
		},
