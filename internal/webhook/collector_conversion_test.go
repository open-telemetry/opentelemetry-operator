	...
	t.Run("valid config", func(t *testing.T) {
		cfgV1 := v1alpha1.OpenTelemetryCollector{
			Spec: v1alpha1.OpenTelemetryCollectorSpec{
				Config: convertTestCollectorCfg,
		...
		assert.Nil(t, err)
		assert.YAMLEq(t, convertTestCollectorCfg, string(yamlCfg))
	})
...
func Test_tov1alpha1_config(t *testing.T) {
	cfg := v1beta1.Config{}
	err := go_yaml.Unmarshal([]byte(convertTestCollectorCfg), &cfg)
	...
	require.NoError(t, err)
	assert.YAMLEq(t, convertTestCollectorCfg, alpha1Col.Spec.Config)
}
...
func Test_tov1beta1AndBack(t *testing.T) {
	...
	colalpha1 := &v1alpha1.OpenTelemetryCollector{
		...
		Spec: v1alpha1.OpenTelemetryCollectorSpec{
			...
			TargetAllocator: createConvertTestTA(),
			...
			Ingress: v1alpha1.Ingress{
				...
				RuleType:    v1alpha1.IngressRuleTypePath,
				Hostname:    "foo.com",
...
func Test_tov1beta1AndBack_deprecated_replicas(t *testing.T) {
	...
	err = OtelColConvertFrom(&colalpha1, &colbeta1)
	require.NoError(t, err)
	assert.Nil(t, colalpha1.Spec.MinReplicas) //nolint:staticcheck
	assert.Nil(t, colalpha1.Spec.MaxReplicas) //nolint:staticcheck
...
func TestConvertTo(t *testing.T) {
	col := v1alpha1.OpenTelemetryCollector{
		ObjectMeta: metav1.ObjectMeta{
			Name: "otel",
		},
		Spec: v1alpha1.OpenTelemetryCollectorSpec{
...
func TestConvertFrom(t *testing.T) {
	...
	col.Spec.Config = ""
	assert.Equal(t, v1alpha1.OpenTelemetryCollector{
		ObjectMeta: metav1.ObjectMeta{
			Name: "otel",
		},
		Spec: v1alpha1.OpenTelemetryCollectorSpec{
