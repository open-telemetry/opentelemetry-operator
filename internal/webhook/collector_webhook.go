...
func (c CollectorWebhook) Default(_ context.Context, otelcol *v1beta1.OpenTelemetryCollector) error {
	...
	events, err := apihelpers.ApplyDefaults(&otelcol.Spec.Config, c.logger)
	if err != nil {
		return err
	}
...
func (c CollectorWebhook) Validate(ctx context.Context, r *v1beta1.OpenTelemetryCollector) (admission.Warnings, error) {
	...
	nullObjects := apihelpers.NullObjects(&r.Spec.Config)
	...
	ports, errPorts := apihelpers.GetAllPorts(&r.Spec.Config, c.logger)
	if errPorts != nil {
	...
	if c.fips != nil {
		components := apihelpers.GetEnabledComponents(&r.Spec.Config)
		if notAllowedComponents := c.fips.DisabledComponents(components[apihelpers.KindReceiver], components[apihelpers.KindExporter], components[apihelpers.KindProcessor], components[apihelpers.KindExtension]); notAllowedComponents != nil {
...
func (c CollectorWebhook) validateTargetAllocatorConfig(ctx context.Context, r *v1beta1.OpenTelemetryCollector) (admission.Warnings, error) {
	...
	cfgYaml, err := apihelpers.Yaml(&r.Spec.Config)
	if err != nil {
		return nil, err
	}
...
func SetupCollectorWebhook(mgr ctrl.Manager, cfg config.Config, reviewer *rbac.Reviewer, metrics *apihelpers.Metrics, bv BuildValidator, fipsCheck fips.FIPSCheck) error {
