...
func TestInstrumentationValidatingWebhook_UnupgradableVersionWarnings(t *testing.T) {
	...
	webhook := InstrumentationWebhook{
		cfg: config.Config{
			AutoInstrumentationJavaImage:   "ghcr.io/org/java:latest",
			AutoInstrumentationPythonImage: "ghcr.io/org/python:latest",
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			warnings, err := webhook.ValidateCreate(t.Context(), &tt.inst)
			assert.Nil(t, err)
