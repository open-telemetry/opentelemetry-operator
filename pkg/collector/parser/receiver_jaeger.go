...
func (j *JaegerReceiverParser) ContainerPorts() ([]corev1.ContainerPort, error) {
	...
	for _, protocol := range []struct {
		...
		if receiverProtocol, ok := j.config[protocol.name]; ok {
			...
			settings, ok := receiverProtocol.(map[interface{}]interface{})
			if ok {
				protocolPort = singleContainerPortFromConfigEndpoint(j.logger, nameWithProtocol, settings)
			}

			// have we parsed a port based on the configuration block?
			// if not, we use the default port
			if protocolPort == nil {
				protocolPort = &corev1.ContainerPort{
					Name:          portName(nameWithProtocol, protocol.defaultPort),
					ContainerPort: protocol.defaultPort,
				}
			}
