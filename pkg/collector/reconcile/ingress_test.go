...
func TestExpectedIngresses(t *testing.T) {
	t.Run("should create and update ingress entry", func(t *testing.T) {
		...
		err = expectedIngresses(ctx, params, []*networkingv1.Ingress{collector.Ingress(params.Config, params.Log, params.Instance)})
...
func TestDeleteIngresses(t *testing.T) {
	t.Run("should delete excess ingress", func(t *testing.T) {
		...
		err = expectedIngresses(ctx, myParams, []*networkingv1.Ingress{collector.Ingress(myParams.Config, myParams.Log, myParams.Instance)})
		assert.NoError(t, err)
