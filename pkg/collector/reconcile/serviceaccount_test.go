...
func TestExpectedServiceAccounts(t *testing.T) {
	t.Run("should create multiple service accounts", func(t *testing.T) {
		param := params()
		desired, err := collector.ServiceAccount(param.Config, param.Log, param.Instance)
		assert.NoError(t, err)
		allocatorDesired, err := targetallocator.ServiceAccount(param.Config, param.Log, param.Instance)
		assert.NoError(t, err)
...
func TestDeleteServiceAccounts(t *testing.T) {
	t.Run("should delete the managed service account", func(t *testing.T) {
		...
		desired, err := collector.ServiceAccount(param.Config, param.Log, param.Instance)
		assert.NoError(t, err)
		err = deleteServiceAccounts(context.Background(), params(), []*v1.ServiceAccount{desired})
		assert.NoError(t, err)
