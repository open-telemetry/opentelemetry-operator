...
// Package reconcile contains reconciliation logic for OpAMPBridge components.
package reconcile
...
func Self(ctx context.Context, params Params) error {
	...
	if err := params.Client.Status().Patch(ctx, &changed, statusPatch); err != nil {
		return fmt.Errorf("failed to apply status changes to the OpAMPBridge CR: %w", err)
	}
