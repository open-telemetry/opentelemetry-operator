...
func Services(ctx context.Context, params Params) error {
	...
	for _, builder := range []builder{desiredService} {
...
func desiredService(ctx context.Context, params Params) *corev1.Service {
	...
	labels["app.kubernetes.io/name"] = naming.OpAMPBridgeService(params.Instance)

	selector := opampbridge.SelectorLabels(params.Instance)
	selector["app.kubernetes.io/name"] = naming.OpAMPBridge(params.Instance)
	...
	ports = append(params.Instance.Spec.Ports, ports...)

	return &corev1.Service{
		ObjectMeta: metav1.ObjectMeta{
			Name:      naming.OpAMPBridgeService(params.Instance),
