...
SEARCH_STRINGS=(
  ...
  "k8s.deployment.name: Str(my-java)"
  "os.type: Str(linux)"
  "process.executable.path: Str(/opt/java/openjdk/bin/java)"
  "service.name: Str(my-java)"
...
while true; do
    ...
    if [ "$ELAPSED_TIME" -ge "$RETRY_TIMEOUT" ]; then
        echo "-----------------------------------------------------"
        echo "ERROR: Timeout ($RETRY_TIMEOUT seconds) reached. Not all required strings were found in pod '$POD'."
        echo "-----------------------------------------------------"
        exit 1
    fi

    echo "-----------------------------------------------------"
    echo "Attempting log check on OTEL collector pod '$POD' (Elapsed: ${ELAPSED_TIME}s / ${RETRY_TIMEOUT}s)"
    ...
    if [ $KUBECTL_LOGS_EXIT_CODE -ne 0 ]; then
         echo "Warning: Failed to get logs in pod '$POD' (Exit code: $KUBECTL_LOGS_EXIT_CODE). Retrying after sleep..."
         sleep "$RETRY_SLEEP"
         continue # Go to next loop iteration
    fi

    if [ -z "$LOGS" ]; then
         echo "Warning: Logs in pod '$POD' were empty on this attempt. Retrying after sleep..."
         sleep "$RETRY_SLEEP"
         continue # Go to next loop iteration
    fi
    ...
    if [ "$ALL_FOUND_THIS_ATTEMPT" = true ]; then
        echo "-----------------------------------------------------"
        echo "Success: All ${#SEARCH_STRINGS[@]} strings found simultaneously in pod '$POD'."
        echo "-----------------------------------------------------"
